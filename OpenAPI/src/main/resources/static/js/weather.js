/* 서비스키(인증키) */
const serviceKey = "DQVPom+PKX263zn1WwCQgKBz3Ad8d+5084fEHs0Tj+kM2sPcO/t3cuJyShe9n9daDfH/V5Teq9f4vtDQjVLHSA==";
...
const getUltraSrtFcst = (regionValue)=>{
  ...
  .then(resp=>resp.json()) //응답 받은 JSON을 JS Object로 반환
  .then(result =>{
    console.log(result);
    ...
    //현재 시간으로부터 가장 가까운 예보를 저장할 빈 객체 생성
    const weatherObj = {};
...
/* select 변경시 해당 지역 날씨 조회 */
region.addEventListener("change",e=>{
  regionName.innerText = e.target.value; //화면 지역명 변경
  getUltraSrtFcst(e.target.value); // 해당지역 날씨 조회
});
