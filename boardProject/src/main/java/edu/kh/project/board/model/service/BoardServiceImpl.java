...
public class BoardServiceImpl implements BoardService {
	...
	public Map<String, Object> selectBoardList(int boardCode, int cp) {
		...
		/* Mapper 메서드 호출 시 
		 * - 첫번째 매개 변수 -> SQL에 전달할 파라미터
		 * - 두번째 매개 변수 -> RowBounds 객체 전달
		 * 
		 * */
	...
	public Board selectOne(Map<String, Integer> map) {
		...
		// 2. 수행하려는 SQL이  
		//	  1) 모두 SELECT 이면서
		//	  2) 먼저 조회된 결과 중 일부를 이용해 나중에 수행되는 
		//		 SQL의 조건으로 삼을 때
		//    --> MyBatis의 <resultMap>,<collection> 태그를 이용해 
		// 		  Mapper 메서드 1회 호출로 여러 SELECT를 한번에 수행 가능
	...
	public int updateReadCount(int boardNo) {
		...
		// 2. 현재 조회 수 조회
		if(result>0) {
			return mapper.selectReadCoun(boardNo);
		}
	...
	public Map<String, Object> searchList(Map<String, Object> paramMap, int cp) {
	    ...
	    * */
		...
		/*
		 * Mapper 메서드 호출 시 - 첫번째 매개 변수 -> SQL에 전달할 파라미터 - 두번째 매개 변수 -> RowBounds 객체 전달
		 * 
		 */
