...
public class ImageDeleteScheduling {
	...
	@Autowired // 서비스 bean 주입(DI)
	private SchedulingService service;
	
	@Value("${my.profile.folder-Path}")
	private String memberLocation; //프로필 이미지 저장 경로
	
	@Value("${my.board.folder-Path}")
	private String boardLocation; //게시글 이미지 저장 경로
	
	//@Scheduled(cron="0/10 * * * * *") //0초 기준 10초마다 (테스트)
	@Scheduled(cron="0 0  * * * *") //정시마다 (0분 0초 마다) (실제 운영 시)
	public void imageDelete() {
	
		// 1. 서버에 폴더 지정 (java.io.File 이용)
		File profileFolder = new File(boardLocation);
		...
		// 4. 비어있는 리스트를 만들어서 변환된 리스트를 한 곳에 저장
		...
		List<String> dbImgeList = service.selectImageList();
		
		// 6. DB 모두 이미지가 존재하지 않을 경우
		if(dbImgeList.isEmpty()) {
			log.info("---------- DB이미지 없으므로 삭제 스케쥴러 종료");
			return;
		}
		...
		for(File server : serverImageList) {
			...
			// 8. 서버 이미지가 DB이미지 목록에 존재하지 않는 경우
			// File.getName() : 파일명 얻어오기
			if(!dbImgeList.contains(server.getName())) {
