...
<input type = "file">문제점
1. 선택 후 취소 -> 파일 없어짐
2. 최대 크기 초과 시 -> 파일이 선택되어 있음
3. value로 대입할 수 있는 값이 빈칸("")밖에 없음
...
// 이미지 선택 이후 취소를 누를 경우르 대비한 백업 이미지
// (백원 원리 -> 복제품으로 기존 요소를 대체함)
...
const changeImageFn =(inputImage,order)=>{
  ...
  if(file == undefined){
    ...
    /* 한번 화면에 추가된 요소는 재사용(다른 곳에 또 추가) 불가능 */

    // 백업본을 한번 더 복제
    const temp = backupInputList[order].cloneNode(true);

    inputImage.after(temp); //백업본을 다음 요소로 추가
    inputImage.remove(); // 원본 삭제
    inputImage = temp; // 원본 변수에 백업본 참조할 수 있게 대입

    // clone은 addEventListener 복사 못함
    // 백업본에 없는 이벤트 리스너 다시 추가
    inputImage.addEventListener("change",e=>{
      changeImageFn(e.target,order); //재귀호출
    })
    return;
  }

  //----------- 선택된 파일의 크기가 최대 크기(max)를 초과 -----------
  if(file.size > maxSize){
    alert("10MB 이하의 이미지를 선택해주세요");

    // 해당 순서의 backup 요소가 없거나,
    // 요소는 있는데 값이 없는 경우 == 아무 파일도 선택된 적 없을 경우
  ...
  reader.addEventListener("load",e=>{
    ...
    //img태그(.preview)에 src 속성으로 url값 대임
...
boardUpdateFrm.addEventListener("submit",e =>{
  ...
  //-> value(문자열) 저장 시 배열은 toString이 호출 되어 양쪽 대괄호[]가 사라지게 됨
