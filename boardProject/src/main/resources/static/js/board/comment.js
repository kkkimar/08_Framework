...
- 자원(데이터,파일)을 이름(주소)으로 구분(representational)하여
  ...
  -> 자원의 이름(주소)를 명시하고 
     ...
     자원의 이름(주소)ㄴ은 하나만 지정 (ex. /comment)
...
const selectCommentList = ()=>{
  ...
  // fetch(주소?쿼리스트링) - GET 방식만 가능

  //[POST,PUT,DELETE]
  // fetch(주소,{method :"", header:{}, body:"")

  // response.json() 
  // - 응답 받은 JSON 데이터 -> JS객체로 변환
  ...
  .then(commentList => {
    ...
    for(let comment of commentList){
      ...
      commentRow.innerText="삭제된 댓글 입니다";
        ...
        //프로필 이미지
        const profileIme = document.createElement("img");

        if(comment.profileImg == null) 
        profileIme.src= userDefaultImg;
        else 
        profileIme.src= comment.profileImg;
        ...
        commentWriter.append(profileIme,nickname,commentDate);
...
addContent.addEventListener("click", e =>{
  ...
  const data ={
    ...
    "boardNo" : boardNo,
    "memberNo" : loginMemberNo // 또는 session 회원 번호 이용도 가능함
  };
...
const showUpdateComment = (commentNo, btn) => {
  ...
  //.update-textarea 존재 == 열려있는 댓글 수정창잉 존재
  if(temp != null){

    if(confirm("수정 중인 댓글이 있습니다. 현재 댓글을 수정 하시겠습니까?")){

      const commentRow = temp.parentElement; //기존 댓글행
      commentRow.after(beforeCommentRow); // 기존 댓글 다음에 백업 추가
      commentRow.remove(); // 기존 삭제 -> 백업이 기존 위치로 이동
    }else{ //취소
      return;
    }
