...
// true == 해당 항목은 유효한 형식으로 작성 됨
// false == 해당 항목은 유효하지 않은 형식으로 작성 됨
...
memberEmail.addEventListener("input", e =>{
  ...
  if(inputEmail.trim().length === 0){
    ...
    // 잘못 입력한 띄어쓰기가 있을 경우 없앰(처음부터 띄어쓰기 입력 막음)
    memberEmail.value ="";
...
memberPw.addEventListener("input", e =>{
  ...
  if(inputPw.trim().length == 0){
    ...
    pwMessage.classList.remove('confirm','error'); // 검은 글씨로 만들기
    checkObj.memberPw = false; //비밀번호가 유효하지 않다고 표시

    memberPw.value=""; //처음 띄어쓰기 입력 못하게 막기
    return; // 호출한 곳이 없으면 끝!
  }
...
memberNickname.addEventListener("input",e =>{
  ...
  if(!regExp.test(inputNick)){// 유효성 실패인 경우
    nickMessage.innerText = "한글,영어,숫자로만 2~10글자로만 입력해 주세요";
...
signUpForm.addEventListener("submit",e=>{
  ...
  // checkObj의 저장된 값(values)중 
  // 하나라도 false가 있는 경우 제출 X

  // for ~ in (객체 전용 향상된 for문)
  for(let key in checkObj){ //checkObj key값을 순서대로 꺼내옴
...
checkAuthKeyBtn.addEventListener("click",()=>{
  ...
  fetch("/email/checkAuthKey",{
    method : "POST",
    headers : {"Content-Type" : "application/json"},
    body: JSON.stringify(obj) //JS 객체는 java에서 인식 못하기때문에 JSON으로 바꿔주기
  })
  .then(resp=>resp.text())
  .then(result =>{
     // ==   : 값만 비교
