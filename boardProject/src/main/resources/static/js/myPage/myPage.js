...
// 요소.cloneNode(true|false) : 요소 복제(true 작성시 하위 요소도 복제) 
let backupInput;
...
if(profile != null){
  ...
  // 엑스 버튼 (프로필 이미지를 제거하고 기본 이미지로 변경하는 요소)
  ...
  const changeImageFn = e=>{
    ...
    if(file == undefined){
      ...
      // 파일 선택 후 취소를 누르면 -> value == ''(빈칸)이 됨
      // 선택한 파일 없음으로 기록됨
      // 아래 백업한 input으로 교체 시켜서 이전 이미지 남아있는 것처럼 보이게 하기
      ...
      // backupInput의 백업본인 temp를 backupInput으로 변경
      backupInput = temp;
  ...
  // ----------- 엑스 버튼 클릭 시 기본 이미지로 변경 ----------
  deleteImage.addEventListener("click",()=>{
    
    // 프로필 이미지(img태그)를 기본 이미지로 변경하기
