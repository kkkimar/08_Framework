...
function getTotalCount(){//함수 정의
  ...
  .then(result=>{
    ...
    //totalCount 요소의 내용을 변경
    totalCount.innerText = result;
  });

}
//getTotalCount() 함수 호출 방법
...
function getCompleteCount(){
  ...
  //첫번째 then의 response : 응답결과, 요청주소, 응답 데이터 등이 담겨있음
  //response.text() : 응답 데이터를 text 형태로 변환

  //두번째 then의 result
  //-첫번째 then에서 text로 변환된 응답 데이터(completeCount값)

  fetch("/ajax/completeCount")
  .then(response=>response.text())
  .then(result => {
    //#completeCount 요소의 내용으로 result값 출력
...
addBtn.addEventListener("click",()=>{
  ...
  fetch("/ajax/add",{
    ...
    body   : JSON.stringify(param) //파람 객체를 JSON(자료형:String)으로 변환
  })
...
const selectTodo =(url)=>{
  ...
  .then(todo=>{
    //매개변수 : todo : 서버 응답(JSON)이 Object로 변환된 값
    //(첫 번째 then 반환 결과)
    console.log(todo);
...
const selectTodoList = ()=>{
  ...
  .then(result => { // result == 첫 번째 then에서 반환된 결과값
    ...
    console.log(todoList); //객체 배열 형태임을 확인!

    //------------------------------------------
    /* 기존에 추가 되어 있던 할 일 목록 모두 삭제 */
    tbody.innerHTML="";
...
deleteBtn.addEventListener("click",()=>{
  ...
  //취소 버튼 클릭시 아무것도 안함
  if(!confirm("정말 삭제 하시겠습니까?")){// 취소 버튼 클릭시 -> true 반환
    return; /* 아래쪽에서 끝내기 -> 아래 해석X */
  }
  ...
  fetch("/ajax/delete",{
    ...
    //데이터 하나를 전달해도 application/json 작성하기
...
changeComplete.addEventListener("click",()=>{
  ...
  fetch("/ajax/changeComplete",{
    method: "PUT",
    headers : {"Content-Type" : "application/json"},
    body : JSON.stringify(obj) //obj를 json으로 변경해서 요청 보내기
  })
  .then(response=>response.text())
  .then(result=>{
    if(result>0){
      ...
      // selectTodo();
      // -> 서버 부하 크기때문에 부하를 줄이기 위해 상세 조회에서 Y/N만 바꾸기
      popupComplete.innerText = complete; 
      
      // getCompleteCount();
      // -> 서버 부하 줄이기 위해 완료된 Todo 개수 +-1
